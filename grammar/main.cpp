...
class TGLex : public LexicalAnalyzer {
 ...
 private:
  ...
  Token token_newline() {
    ...
    unget();
    if (c == ' ')
      error("Spaces are not allowed at the start of a new line.");
    if (tabs < _tabs) {
...
int main() {
  Translation t(
    std::make_unique<TGLex>(), "lscelr", ctfgc::grammar, std::make_unique<TGOutput>("test"));
  t.run(std::cin, std::cout, std::cerr, "stdin", ctfgc::to_string);
}
