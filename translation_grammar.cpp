...
void TranslationGrammar::create_first(const vector<bool> &empty, vector<vector<Terminal>> &first)
{
    for (auto &n: nonterminals_)
    {
        ...
        for(auto it = nonterminals_.begin(); it != nonterminals_.end(); ++it)
        {
            ...
            for (auto &r: rules_[n])
            {
                if(modify_first(rfirst, r.input()[0]))
                    changed = true;
                for(auto nit = r.input().begin() + 1;it != r.input().end() && empty[nonterm_index(*nit)]; ++it)
                {
...
bool TranslationGrammar::modify_first(vector<Terminal> &target, const Symbol &symbol)
{
    switch(symbol.type)
    {
    case Symbol::Type::TERMINAL:
...
bool TranslationGrammar::modify_set(vector<Terminal> &target, const vector<Terminal> &addition)
{
...
void TranslationGrammar::create_follow(const vector<bool> &empty, const vector<vector<Terminal>> &first, vector<vector<Terminal>> &follow)
{
    for (auto &n: nonterminals_)
    {
        follow.push_back(vector<Terminal>());
    }
    follow[nonterm_index(starting_symbol_.value.nonterminal)].push_back(Terminal::EOI());
...
bool TranslationGrammar::rule_follow(const Rule &r, const vector<bool> &empty, const vector<vector<Terminal>> &first, vector<vector<Terminal>> &follow)
{
    bool changed = false;
    for(auto it = r.input().begin(); it < r.input().end(); ++it)
    {
        if(it->type == Symbol::Type::NONTERMINAL)
        {
            ...
            for(auto it2 = it + 1; it != r.input().end(); ++it2)
            {
                ...
                if(it2->type == Symbol::Type::TERMINAL || empty[i2] == false)
                {
                    isepsilon = false;
                }
            }
...
void create_predict(const vector<bool> &empty, const vector<vector<Terminal>> &first, const vector<vector<Terminal>> &follow, vector<vector<Terminal>> &predict)
{
    for(auto &n: nonterminals_)
    {
        for(auto &r: rules_[n])
        {
            vector<Terminal> groupfirst;
            vector<Terminal> rfollow = follow[nonterm_index(r.nonterm().value.nonterminal)];
            bool isEmpty = true;
            for(auto &s: r.input())
            {
...
LLTable create_ll(const vector<bool> &empty, const vector<vector<Terminal>> &first, const vector<vector<Terminal>> &follow)
{
